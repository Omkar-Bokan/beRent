...
const getAdminProfile = async (req, res) => {
    ...
    try {
        const profile = await Admin_1.AdminProfile.findOne({}); // Find the first (and ideally only) admin profile
        if (!profile) {
...
const updateAdminProfile = async (req, res) => {
    ...
    try {
        const profile = await Admin_1.AdminProfile.findOneAndUpdate({}, // Query to find the document (e.g., find the first one)
